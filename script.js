document.addEventListener('DOMContentLoaded', () => {
  ...
  function analyzeText(text) {
    ...
    // Homoglyphs (examples, not exhaustive)
    const homoglyphs = {
  ...
  function displayResults(suspiciousChars, visuallySimilar) {
    ...
    if (suspiciousChars.length > 0) {
        ...
        suspiciousList.className = 'result-section__list'; // Add this class
        suspiciousChars.forEach(result => {
            const listItem = document.createElement('li');
            listItem.className = 'result-section__item'; // Add this class
            listItem.innerHTML = `
    ...
    if (visuallySimilar.length > 0) {
        ...
        similarList.className = 'result-section__list'; // Add this class
